...
module.exports.process = function (element, opts) {
    ...
    if (['text'].indexOf(element.type) > -1 && element.data.length > 0) {
        ...
        var lineColFunc = knife.getLineColFunc(element.data, element.openLineCol);
    ...
    if (element.attribs) {
        ...
        for (var index = 0; index < attributeNames.length; index++) {
            ...
            var lineColFunc = knife.getLineColFunc(valueObject.value, valueObject.valueLineCol);
            [regex.improper, regex.brackets, regex.unescaped].forEach(function (currentRegex) {
                ...
                while (match) {
                    var lineCol = lineColFunc(match.index);
                    issues.push({
                        index: valueObject.valueIndex + match.index,
                        line: lineCol[0],
                        column: lineCol[1],
                        msg: message + match[1]
                    });
